...
func generateRecoveryCode() string {
	...
	if _, err := rand.Read(randomBytes); err != nil {
		// Fallback should ideally panic or return error, but signature returns string
		// Since rand.Read rarely fails unless OS crypto entropy is depleted, panic is safer than predictable code
...
// isBuggyOldCode detects codes generated by the old flawed time.Now() implementation 
// which always produced a string of identical characters (e.g., SSSS-SSSS-SSSS-SSSS or JJJJ-JJJJ-JJJJ-JJJJ)
func isBuggyOldCode(code string) bool {
	if len(code) != 19 {
		return true // invalid format anyway
	}
	firstChar := code[0]
...
func (s *service) ResetPassword(ctx context.Context, recoveryCode, newPassword string) error {
	...
	// Normalize both codes for comparison (remove hyphens, convert to uppercase)
...
func (s *service) Login(ctx context.Context, password string) (*models.Session, error) {
	user, err := s.repo.GetFirstUser(ctx)
	if err != nil {
		// If no user exists, let's create the default one for the workspace
	...
	// Safe one-time regeneration path for the existing local development user
	if user.RecoveryCode != nil && isBuggyOldCode(*user.RecoveryCode) {
		newCode := generateRecoveryCode()
